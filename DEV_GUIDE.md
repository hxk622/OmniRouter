...
| 项目 | 说明 |
|------|------|
| **上游仓库** | hxk622/OmniRouter |
| **Fork 仓库** | bayma888/omnirouter-bmai |
...
| 数据库凭据 | user=`omnirouter`, password=`omnirouter`, dbname=`omnirouter` |
...
# 同步当前环境配置（默认 dev，可通过 APP_ENV/OMNI_ENV 切换）
pnpm run env:sync
...
psql -U omnirouter -h 127.0.0.1 -d omnirouter -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER omnirouter WITH PASSWORD 'omnirouter';
...
```bash
# 连接数据库
psql -U omnirouter -h 127.0.0.1 -d omnirouter
...
# 执行 SQL 文件
psql -U omnirouter -h 127.0.0.1 -d omnirouter -f migration.sql
```
...
- [上游仓库](https://github.com/hxk622/OmniRouter)
