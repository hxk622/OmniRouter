#!/bin/bash
#
# OmniRouter Installation Script
# OmniRouter 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/hxk622/OmniRouter/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="hxk622/OmniRouter"
INSTALL_DIR="/opt/omnirouter"
SERVICE_NAME="omnirouter"
SERVICE_USER="omnirouter"
CONFIG_DIR="/etc/omnirouter"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="OmniRouter 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 OmniRouter 服务："
    ...
    ["not_installed"]="OmniRouter 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 OmniRouter。"
    ...
    ["uninstall_complete"]="OmniRouter 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 OmniRouter"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 OmniRouter"
    ...
    ["server_config_desc"]="配置 OmniRouter 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="OmniRouter Installation Script"
    ...
    # Completion
    ["install_complete"]="OmniRouter installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start OmniRouter service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading OmniRouter..."
    ...
    ["not_installed"]="OmniRouter is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove OmniRouter from your system."
    ...
    ["uninstall_complete"]="OmniRouter has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install OmniRouter"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove OmniRouter"
    ...
    ["server_config_desc"]="Configure OmniRouter server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/omnirouter" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/omnirouter" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="omnirouter_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/omnirouter" "$INSTALL_DIR/omnirouter"
    chmod +x "$INSTALL_DIR/omnirouter"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/omnirouter"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/omnirouter.service << EOF
[Unit]
Description=OmniRouter - AI API Gateway Platform
Documentation=https://github.com/hxk622/OmniRouter
...
User=omnirouter
Group=omnirouter
WorkingDirectory=/opt/omnirouter
ExecStart=/opt/omnirouter/omnirouter
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=omnirouter
...
start_service() {
    ...
    if systemctl start omnirouter; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u omnirouter -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable omnirouter 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status omnirouter"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u omnirouter -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart omnirouter"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop omnirouter"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if OmniRouter is installed
    if [ ! -f "$INSTALL_DIR/omnirouter" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/omnirouter" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet omnirouter; then
        print_info "$(msg 'stopping_service')"
        systemctl stop omnirouter
    fi

    # Backup current binary
    cp "$INSTALL_DIR/omnirouter" "$INSTALL_DIR/omnirouter.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/omnirouter.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/omnirouter"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start omnirouter

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: OmniRouter must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/omnirouter" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="omnirouter.backup.${current_version}"
        else
            backup_name="omnirouter.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/omnirouter" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop omnirouter 2>/dev/null || true
    systemctl disable omnirouter 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/omnirouter.service
    systemctl daemon-reload
