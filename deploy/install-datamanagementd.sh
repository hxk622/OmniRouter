...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/omnirouter/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/omnirouter"
DATA_DIR="/var/lib/omnirouter/datamanagement"
SERVICE_FILE_NAME="omnirouter-datamanagementd.service"
...
参数:
  ...
  --source  指定 omnirouter 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/omnirouter-src
EOF
}
...
if ! id omnirouter >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 omnirouter..."
  useradd --system --no-create-home --shell /usr/sbin/nologin omnirouter
else
  echo "[2/6] 系统用户 omnirouter 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R omnirouter:omnirouter /var/lib/omnirouter
chmod 0750 "$DATA_DIR"
...
systemctl enable --now omnirouter-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status omnirouter-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u omnirouter-datamanagementd -f
2. 在 omnirouter（容器部署时）挂载 socket:
