# OmniRouter Deployment Files

This directory contains files for deploying OmniRouter on Linux servers.
...
| `.env.dev` / `.env.test` / `.env.prod` | Environment profiles for development, test, and production |
| `DOCKER.md` | Docker Hub documentation |
| `install.sh` | One-click binary installation script |
| `install-datamanagementd.sh` | datamanagementd 一键安装脚本 |
| `omnirouter.service` | Systemd service unit file |
| `omnirouter-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/hxk622/OmniRouter/main/deploy/docker-deploy.sh | bash

# Or download first, then run
curl -sSL https://raw.githubusercontent.com/hxk622/OmniRouter/main/deploy/docker-deploy.sh -o docker-deploy.sh
chmod +x docker-deploy.sh
./docker-deploy.sh
```

**What the script does:**
- Downloads `docker-compose.local.yml` and `.env.prod`
...
```bash
# Clone repository
git clone https://github.com/hxk622/OmniRouter.git
cd omnirouter/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker-compose logs omnirouter | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/omnirouter-datamanagement.sock`
...
# Restart OmniRouter only
docker-compose -f docker-compose.local.yml restart omnirouter
...
# View logs
docker-compose logs -f omnirouter

# Restart OmniRouter only
docker-compose restart omnirouter
...
| `ADMIN_EMAIL` | No | `admin@omnirouter.dev` | Admin email |
| `ADMIN_PASSWORD` | No | `OmniRouterAdmin123!` | Admin password |
...
See `./.env.dev`, `./.env.test`, or `./.env.prod` for all available options.
...
docker-compose -f docker-compose.local.yml down
cd ..
tar czf omnirouter-complete.tar.gz deployment/

# Transfer to new server
scp omnirouter-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf omnirouter-complete.tar.gz
cd deployment/
docker-compose -f docker-compose.local.yml up -d
```
...
OmniRouter supports three methods to connect to Gemini:
...
```bash
curl -sSL https://raw.githubusercontent.com/hxk622/OmniRouter/main/deploy/install.sh | sudo bash
```

### Manual Installation

1. Download the latest release from [GitHub Releases](https://github.com/hxk622/OmniRouter/releases)
2. Extract and copy the binary to `/opt/omnirouter/`
3. Copy `omnirouter.service` to `/etc/systemd/system/`
4. Run:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable omnirouter
   sudo systemctl start omnirouter
   ```
...
# Stop the service
sudo systemctl stop omnirouter

# Restart the service
sudo systemctl restart omnirouter

# Check status
sudo systemctl status omnirouter

# View logs
sudo journalctl -u omnirouter -f
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit omnirouter
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/omnirouter.service
   ```
...
The main config file is at `/etc/omnirouter/config.yaml` (created by Setup Wizard).
...
```
/opt/omnirouter/
├── omnirouter              # Main binary
├── omnirouter.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker-compose -f docker-compose.local.yml logs --tail=100 omnirouter
...
# View detailed logs
docker-compose logs --tail=100 omnirouter
...
# View recent logs
sudo journalctl -u omnirouter -n 50

# Check config file
sudo cat /etc/omnirouter/config.yaml
...
OmniRouter supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.omnirouter.org](https://tls.omnirouter.org/)** to get TLS fingerprint information for different devices and browsers.
