...
const (
	testGeminiBuiltinClientID     = "test-gemini-built-in-client-id"
	testGeminiBuiltinClientSecret = "test-gemini-built-in-client-secret"
)

func setTestGeminiBuiltinOAuthConfig(t *testing.T) {
	t.Helper()
	...
	t.Setenv(GeminiCLIOAuthClientIDEnv, testGeminiBuiltinClientID)
	t.Setenv(GeminiCLIOAuthClientSecretEnv, testGeminiBuiltinClientSecret)
	t.Cleanup(func() {
...
func clearTestGeminiBuiltinOAuthConfig(t *testing.T) {
	t.Helper()
...
func TestBuildAuthorizationURL(t *testing.T) {
	setTestGeminiBuiltinOAuthConfig(t)
	...
	checks := []string{
		"response_type=code",
		"client_id=" + testGeminiBuiltinClientID,
		"redirect_uri=",
		"state=test-state",
...
func TestBuildAuthorizationURL_UsesBuiltinConfigFromEnv(t *testing.T) {
	setTestGeminiBuiltinOAuthConfig(t)
	...
	if !strings.Contains(authURL, "client_id="+testGeminiBuiltinClientID) {
...
func TestEffectiveOAuthConfig_NoBuiltinEnv(t *testing.T) {
	clearTestGeminiBuiltinOAuthConfig(t)

	_, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err == nil {
		t.Fatal("未配置内置 client_id/client_secret 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) || !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("错误消息应提及两个环境变量，实际: %v", err)
	}
}
