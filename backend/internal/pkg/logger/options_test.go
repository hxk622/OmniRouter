...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/omnirouter-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/omnirouter-data", "logs", "omnirouter.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "omnirouter.log")
